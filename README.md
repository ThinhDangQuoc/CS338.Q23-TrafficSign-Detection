...
- Tùy chỉnh ngưỡng confidence, inference size và tần suất xử lý frame để cân bằng tốc độ/độ chính xác.
- Hiển thị inference time, FPS ước tính và thống kê detection.
...
- Có tab đánh giá model, đọc các biểu đồ và `results.csv` YOLO nếu có.
- Có webcam realtime bằng `streamlit-webrtc` ở mức demo đồ án.
...
- streamlit-webrtc và av cho webcam realtime
...
Streamlit hiển thị kết quả
        ↓
Hiển thị FPS / inference time / thống kê detection
        ↓
...
Nếu dùng webcam realtime, hãy chạy app trên máy local có camera và cấp quyền camera trong trình duyệt. Hai dependency chính cho webcam là `streamlit-webrtc` và `av`, đã được khai báo trong `requirements.txt`.
...
Trong project này, model từ `trained_weights/best.pt` đã được copy sang `models/best.pt`. Nếu bạn thay model khác, hãy đặt file YOLO `.pt` vào đúng vị trí trên.
...
### Sidebar cấu hình detect

Sidebar có các tuỳ chọn chính:

- Ngưỡng confidence.
- Image size / inference size: `416`, `640`, `960`.
- Process every N frames cho video và webcam.
- Bật/tắt lưu lịch sử nhận diện.
- Bật/tắt TTS.
- Bật/tắt hiển thị FPS / inference time.
- Chọn loại phương tiện để xem mức phạt tham khảo.

### Detect ảnh

Upload ảnh, chạy YOLO, hiển thị ảnh gốc và ảnh đã vẽ bounding box. App đo inference time, FPS ước tính, số detection và confidence trung bình. Mỗi detection hiển thị class ID, tên biển, confidence, loại biển, ý nghĩa, hành động cần làm, cảnh báo, lỗi thường gặp, mức phạt tham khảo và ví dụ.

### Detect video

Upload video, xử lý từng frame theo khoảng cách N frame để giảm lag, hiển thị frame đã vẽ bounding box và lưu lịch sử có cooldown để tránh trùng liên tục. Sau khi xử lý, app hiển thị FPS inference trung bình, tổng frame đã detect, tổng detection và thời gian xử lý.

### Webcam realtime

Tab `Webcam realtime` dùng `streamlit-webrtc` để nhận frame từ camera trình duyệt, detect bằng YOLO, vẽ bounding box, hiển thị tên biển/confidence và overlay FPS nếu bật. Lịch sử và TTS đều có cooldown để tránh lưu hoặc đọc lặp liên tục.

Nếu thiếu `streamlit-webrtc` hoặc `av`, tab webcam sẽ hiển thị hướng dẫn cài dependency thay vì làm app crash.

### Đánh giá model

Tab `Đánh giá model` hiển thị:

- Model path: `models/best.pt`.
- Số class đọc từ `data/classes.txt`.
- Dataset: Vietnamese Traffic Signs.
- Framework: YOLO.
- Trạng thái model: loaded / missing / error.
- Kích thước file model nếu đọc được.

App tự tìm các file kết quả YOLO trong:
...
Nếu có, app hiển thị `results.png`, confusion matrix, PR/P/R/F1 curves và đọc `results.csv` bằng pandas. Với `results.csv`, app hiển thị bảng metrics, metric cuối cùng nếu tìm được Precision, Recall, mAP50, mAP50-95 và biểu đồ line đơn giản.
...
SQLite lưu lịch sử tại `database/app.db`. Tab lịch sử hiển thị bảng detection, tổng số lần nhận diện, biển xuất hiện nhiều nhất, confidence trung bình, lần nhận diện gần nhất, clear history và export CSV. Có thể tắt lưu lịch sử từ sidebar khi chỉ muốn demo inference.
...
Nếu dùng virtualenv của project:
...
- Video và webcam trong Streamlit là demo gần realtime, chưa tối ưu như hệ thống camera production.
- Webcam phụ thuộc trình duyệt, quyền camera, môi trường local và WebRTC.
- App không thay thế hệ thống an toàn thực tế trên xe.

## Hướng phát triển

- Tối ưu webcam realtime ổn định hơn trên nhiều trình duyệt/thiết bị.
- Cập nhật luật và mức phạt từ nguồn chính thống.
- Tối ưu tốc độ inference, batch frame, dùng GPU hoặc export ONNX/TensorRT.
